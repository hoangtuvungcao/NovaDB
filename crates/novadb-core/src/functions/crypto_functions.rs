...
pub fn register(connection: &Connection) -> Result<()> {
    ...
    connection.create_scalar_function("hmac_sha256", 2, deterministic, |ctx| {
        ...
        let mut mac = HmacSha256::new_from_slice(key.as_bytes())
            .map_err(|e| rusqlite::Error::UserFunctionError(format!("hmac key error: {e}").into()))?;
    ...
    connection.create_scalar_function("base64_decode", 1, deterministic, |ctx| {
        ...
        let decoded = BASE64_STANDARD.decode(s.trim())
            .map_err(|e| rusqlite::Error::UserFunctionError(format!("invalid base64: {e}").into()))?;
    ...
    connection.create_scalar_function("hex_encode", 1, deterministic, |ctx| {
        ...
        Ok(Some(bytes.iter().map(|b| format!("{b:02x}")).collect::<String>()))
    })?;
    ...
    connection.create_scalar_function("hex_decode", 1, deterministic, |ctx| {
        ...
        let clean = s.trim();
        if clean.len() % 2 != 0 {
            return Err(rusqlite::Error::UserFunctionError("hex string must have even length".to_string().into()));
        }
        let mut bytes = Vec::with_capacity(clean.len() / 2);
        for i in (0..clean.len()).step_by(2) {
            let byte = u8::from_str_radix(&clean[i..i+2], 16)
                .map_err(|e| rusqlite::Error::UserFunctionError(format!("invalid hex byte: {e}").into()))?;
            bytes.push(byte);
        }
        Ok(bytes)
    })?;
...
mod tests {
    ...
    fn test_crypto_hashes() {
        let conn = setup();
        let m: String = conn.query_row("SELECT md5('hello')", [], |r| r.get(0)).unwrap();
        assert_eq!(m, "5d41402abc4b2a76b9719d911017c592");

        let s1: String = conn.query_row("SELECT sha1('hello')", [], |r| r.get(0)).unwrap();
        assert_eq!(s1, "aaf4c61ddcc5e8a2dabede0f3b482cd9aea9434d");

        let s256: String = conn.query_row("SELECT sha256('hello')", [], |r| r.get(0)).unwrap();
        assert_eq!(s256, "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824");
    }

    #[test]
    fn test_base64_and_hex() {
        let conn = setup();
        let b64: String = conn.query_row("SELECT base64_encode('NovaDB Engine')", [], |r| r.get(0)).unwrap();
        assert_eq!(b64, "Tm92YURCIEVuZ2luZQ==");

        let dec: String = conn.query_row("SELECT base64_decode('Tm92YURCIEVuZ2luZQ==')", [], |r| r.get(0)).unwrap();
        assert_eq!(dec, "NovaDB Engine");

        let hex_enc: String = conn.query_row("SELECT hex_encode(x'DEADBEEF')", [], |r| r.get(0)).unwrap();
